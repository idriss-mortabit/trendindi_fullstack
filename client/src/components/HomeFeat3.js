...
class HomeFeat3 extends React.Component {

    render() {
        return this.props.articlel !== undefined ? (
		<section id="mvp_home_feat2_widget-4" className="mvp-widget-home left relative mvp_home_feat2_widget">
			<div className="mvp-main-box">
				<div className="mvp-widget-home-head">
					<h4 className="mvp-widget-home-title">
						<span className="mvp-widget-home-title">Business</span>
					</h4>
				</div>
					...
					<div className="mvp-widget-feat2-out left relative">
									...
									<a href={article.Link} onClick={() => {this.props.dispatch(addArticleToCart(article))}} rel="noopener noreferrer" target="_blank">
										<div className="mvp-widget-feat2-left-cont left relative">
											<div className="mvp-feat1-feat-img left relative">
												<img width="560" height="600" src={article.Imagelink} className="attachment-mvp-port-thumb size-mvp-port-thumb wp-post-image" alt=""/>																																						</div>
						...
						<div className="mvp-widget-feat2-side left relative">
							...
							<div className="mvp-widget-feat2-side-list left relative">
											...
											<div className="mvp-feat1-list-out relative">
												...
												<div className="mvp-feat1-list-in">
													<div className="mvp-feat1-list-text">
														<div className="mvp-cat-date-wrap left relative">
															<span className="mvp-cd-cat left relative">{article.Category}</span><span className="mvp-cd-date left relative">{age}</span>
														</div>
														<h2>{article.Title}</h2>
													</div>
												</div>
								...
								<Link onClick={() =>{this.props.dispatch(addKeywordToCart('more_business'))}} to={"/news/business"}>
									<div className="mvp-widget-feat2-side-more-but left relative">
										<span className="mvp-widget-feat2-side-more">More Business</span><i className="fa fa-long-arrow-right" aria-hidden="true"></i>
									</div>
								</Link>
							</div>
						</div>
...
const mapStateToProps =  (state, props) =>  {
	...
	const allarticles =  state.article.articles.filter(article => article.Category === 'business');
	const mainarticle =  allarticles.filter(article => article.Place === 'main');
	const articler =  allarticles.filter(article => article.Place === 'sidel');
	const articlel =  allarticles.filter(article => article.Place === 'sider');
	
	return {
		articlel, mainarticle, articler
	}
}
export default connect(mapStateToProps, null)(HomeFeat3);